...
def create_visualization_window():
    ...
    notebook.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.95, relheight=0.95)

    # Create a notebook for tabular grid layout
    # notebook = ttk.Notebook(root)
    # notebook.place(relx=0.5, rely=0.5, anchor="center", relwidth=0.95, relheight=0.9)

    # Create scrollable tabs for each visualization type
    def create_scrollable_tab(notebook, title):
        tab = tk.Frame(notebook, bg="#001100")  # Background set to black
        notebook.add(tab, text=title)

        scroll_canvas = tk.Canvas(tab, bg="#001100", highlightthickness=2, highlightbackground="#00FF00")  # Green border
    ...
    # Function to create a new window for zoomed visualization
    def create_zoom_window(plot_function, column):
        ...
        zoom_window.configure(bg="#001100")  # Background set to black
        fig = plot_function(column)
        canvas = FigureCanvasTkAgg(fig, master=zoom_window)
        canvas.draw()
        canvas.get_tk_widget().pack(fill="both", expand=True)

    # Render plots with dropdown menus
    def render_plot_with_dropdown(tab, plot_function, column_options, default_column):
        frame = tk.Frame(tab, bg="#001100", relief="sunken", borderwidth=2, highlightbackground="#00FF00")  # Green border
        ...
        # Add zoom and new window buttons
    ...
    navigation_frame=tk.Frame(root,bg="#001100")  # Background set to black
    navigation_frame.pack(fill="x",pady=10)
    proceed_button=ttk.Button(
        navigation_frame,
        text="▶️ Proceed",
        style="Custom.TButton",
        command=lambda: subprocess.Popen(["python3", "/home/sky/Desktop/DataBuddy/trainermodule.py"])  # Launch trainermodule.py
    )
